...
//!     let workspace = WorkSpace::create(&PathBuf::from("/tmp/workspaces"))?;
//!
...
//!
//! # async fn run() -> eyre::Result<()> {
//! // Instantiate a new workspace with base directory
...
impl Workspace {
    ...
    pub async fn load(basedir: &PathBuf) -> Result<Self> {
        ...
        trace!("Looking at all directories");
        let mut dirs = vec![];
...
async fn main() -> Result<()> {
    init_logging("ws", false, true, None)?;

    println!("me PID={}", std::process::id());
    let arg = std::env::args().nth(1).unwrap();
    let ws = Workspace::load(&PathBuf::from(arg)).await?;
    let list = ws
        .list()
        .await?
        .iter()
