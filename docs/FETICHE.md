...
Updated: Mon May 19 14:25:42 CEST 2025

## Current Design

`Fetiche` as a framework was mainly born as a refactor of the code used by the `cat21conv` utility which is the rewrite
in [RUST] of the original Shell script. Code to handle formats and sites (aka sources) were moved into libraries.

As the ACUTE project evolved, its needs did as well, and more code was added to handle these.

`Fetiche` has 4 main library components so far:
...
This is the heart of fetiche: a library that implements an actor-based software component to fetch or stream data from
various sources.
It is fully async. It now incorporates the sources crate.

It can run in two different modes: standalone (as used by `acutectl`) or in daemon-mode (inside `fetiched`). The main
difference is that in daemon mode, it can run multiple tasks asynchronously with a set or workers.

Engine uses a configuration file called `engine.hcl` in the base directory.
...
More details in the [Engine README.md](../engine/README.md).

### Formats (managed in the `fetiche-formats` crate)

This crate implements the various data models used by the different sources. Included are three [ASTERIX]-like
formats --
...
HCL is used for the main "job" language now, starting with Engine v0.25.

### HCL

The current scripts look like this one:
